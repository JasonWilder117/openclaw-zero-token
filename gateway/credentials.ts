...
export function resolveGatewayCredentialsFromConfig(params: {
  ...
  if (trimToUndefined(params.urlOverride) && params.urlOverrideSource !== "env") {
    // Security: credentials are intentionally NOT forwarded to externally-supplied
    // URLs to prevent accidental token leakage to untrusted hosts.
    // Use OPENCLAW_GATEWAY_URL (env source) instead of --gateway-url if you need
    // credentials forwarded to a trusted remote host.
