...
export type AuthorizeGatewayConnectParams = {
  ...
  /**
   * X-Real-IP fallback is disabled by default to prevent IP spoofing.
   * Do NOT set to true without a trusted-proxy config in place.
   */
...
export async function authorizeGatewayConnect(
  ...
  // Security: warn loudly when no rate limiter is provided so callers cannot
  // silently skip brute-force protection in production.
  if (!params.rateLimiter && process.env.NODE_ENV !== "test") {
  ...
  // Security: allowRealIpFallback is locked to false to prevent X-Real-IP spoofing.
  // Passing true would allow an attacker to forge local-IP classification.
  const localDirect = isLocalDirectRequest(
    req,
    trustedProxies,
    false, // allowRealIpFallback: NEVER enable without a trusted-proxy config
  );
  ...
  // Security: auth.mode=none is blocked unless OPENCLAW_ALLOW_AUTH_NONE=1 is
  // explicitly set in the environment. This prevents silent misconfiguration
  // from opening the gateway to unauthenticated access.
  if (auth.mode === "none") {
    if (process.env.OPENCLAW_ALLOW_AUTH_NONE !== "1") {
      return {
        ok: false,
        reason: "auth_mode_none_disabled",
      };
    }
  ...
  const ip =
    params.clientIp ??
    resolveRequestClientIp(req, trustedProxies, false) ??
